...
export default function ShowOrders() {
  ...
  const handleOrderStatus = async (orderId, productStatus) => {
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/api/order/orderStatus`,
        {
          method: "PUT",
          headers: {
      ...
      if (res.status === 200) {
        setOrdersStatus((prev) => ({
          ...prev,
          [data.updatedOrder._id]: data.updatedOrder.orderedItem[0].status,
        }));
        setIsLoading(false);
      }
    } catch (error) {
  ...
  return (
    <Container className="admin-panel">
      <Toaster></Toaster>
      <h2 style={{ marginTop: "5%" }} className="order-title">
        Orders
      </h2>
      {isLoading ? (
        <Spinner width="100%" height="50vh"></Spinner>
      ) : (
        <>
          {orders.length > 0 ? (
                      ...
                      <Container className="item">
                        ...
                        <Container className="item-info">
                          <Container>
                            ...
                            <h2 className="info">{item.productId.name}</h2>
                          </Container>
                          <Container>
